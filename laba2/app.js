document.addEventListener("DOMContentLoaded", () => {
    ...
    function initLFSR() {
        ...
        alert(
            "Ошибка: начальное состояние должно содержать ровно 31 бит (только 0 и 1)",
        );
        return false;
        }
    ...
    function displayBits(element, data, title) {
        ...
        if (data.length >= 4) {
        const firstBits = getBits(data, 0, 40);
        ...
        result += firstBits.substring(32, 40) + "\n";
        } else {
        const firstBits = getBits(data, 0, 40);
    ...
    function displayKeyBits(keyStream) {
        ...
        const firstBits = getBits(keyStream, 0, 40);
        result += firstBits.substring(0, 8) + " ";
        result += firstBits.substring(8, 16) + " ";
        result += firstBits.substring(16, 24) + " ";
        result += firstBits.substring(24, 32) + " ";
    ...
    function handleProcess(mode) {
        ...
        resultOutput,
        result,
        mode === "encrypt" ? "зашифрованный файл" : "расшифрованный файл",
        );
        ...
        saveBtn.textContent =
        mode === "encrypt"
            ? "Сохранить зашифрованный"
            : "Сохранить расшифрованный";
    ...
    saveBtn.addEventListener("click", () => {
        ...
        if (dotIndex === -1) {
            ...
            fileName =
            currentFile.name.substring(0, dotIndex) +
            "_encrypted" +
            currentFile.name.substring(dotIndex);
        }
        } else {
